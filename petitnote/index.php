...
function post(){
	...
	//多重送信防止
	return redirect("./?resno={$resno}&resid={$time}#{$time}");
...
function img_replace(){
	...
	return redirect("./?resno={$no}&resid={$time}#{$time}");
...
function edit(){
	...
	return redirect("./?resno={$no}&resid={$_time}#{$_time}");
...
function search(){
	...
	while ($log = fgets($fp)) {
		...
		while($line=fgets($rp)){
			...
			$lines=explode("\t",$line);
			list($no,$sub,$name,$verified,$com,$url,$imgfile,$w,$h,$thumbnail,$painttime,$log_img_hash,$tool,$pchext,$time,$first_posted_time,$host,$userid,$hash,$oya)=$lines;
