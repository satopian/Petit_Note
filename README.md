...
##  動作環境
- PHP5.6以上の環境が必要です。  
PHP5.6,PHP7.2,PHP8.2,PHP8.3で動作確認しています。  
...
このスクリプトの動作環境はPHP5.6-PHP8.xです。推奨はPHP8.0以上です。
...
- 管理パスによる閲覧注意への切り替え時は、IDを変更しなくなりました。  
閲覧注意への切り替えのみでユーザーによる投稿の内容が変更されない場合はホスト名を変更しないようにしていましたが、IDは管理者のものに切り替わってしまっていました。
- 必要の無いコードを整理しました。
- 編集モード時の`res.html`のコメント欄の開閉の制御を`index.html`にまとめした。
...
これにより、URLを貼り付ける事を目的としたスパムはURLを貼る事ができないため、来なくなる事が予想できます。
