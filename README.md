...
##  動作環境
- PHP5.6以上の環境が必要です。  
PHP5.6,PHP7.2,PHP8.1, PHP8.2.0RC4で動作確認しています。  
...
#### 閉じたスレッドでも、管理人は投稿可に
- 指定日数を経過して閉じたスレッド、レス数が上限に達して閉じたスレッドでも管理者投稿モードでログインしていれば返信(投稿)が可能になりました。  
多くの機能の制限を指定しても管理者は制限を受けない仕様になりました。
#### ログファイルの肥大化防止
- 全体ログに保存されている本文のコメントを全体ログから読みだしている箇所はありません。  
いっそ、全体ログにはコメントを保存しないようにする事も考えましたが、どんなコメントが入っているスレッドなのか把握できるようにするため、120バイト分だけ保存して、実体のコメントの読み込みは各スレッドの個別ログから行うようにしました。  
これによりログファイルの肥大化の進行を遅らせる事ができます。  
すべてのログを1つのファイルにすべて保存するPOTI-boardでは動作が重くなる数の書き込み数でも一定程度軽快さを維持して動作できるようになりました。
#### 負荷削減
- 負荷削減作として、数字の0と文字列の0の時、そして、ブランク''の時はhtmlspecialchars()関数に文字列が入る前に'0'や'ブランク''をreturnするようになりました。  
...
.psdファイルや.chiファイルは20MBぐらいまで肥大化する事がありますが、サーバのphp.iniで上限が5MBになっている事が多いため、各自でphp.iniの設定の許容値を大きくして運用していただく事をおすすめします。  
どうして大きく設定しておいたほうがいいのかというと、NEOの動画ファイル、ChickenPaintのレイヤー情報ファイル、Klecksのpsdファイルサイズがサーバの上限値を超えてしまった時は、画像だけ送信する仕様になっていますので、レイヤーの上に蓋をして、下の絵を隠しておく一次保存の時に、レイヤー情報が消失して下の絵が消えてしまう可能性があるからです。
...
頻繁に変更したくない方がほどんどだと思いますので、更新しなくても大丈夫です。
英語圏では、`let's`のように、単語の省略系にシングルクオートの文字を使用する事があるため、文字列の区切りにシングルクオートが適さない例があるというだけです。  
英語圏の使用を想定した変更点になります。  
