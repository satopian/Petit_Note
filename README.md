...
### 細部の調整
- save.inc.php 
エラーメッセージにHTML特殊文字のエスケープ処理を追加しました。
元から用意されている定型文を出力しているだけなので問題ないはずですがすべての出力にエスケープを実施するという考え方で作業するならエスケープするべき箇所です。
- functions.php
...
これにより、拡張子を省略した、   
`"./?mode=saveimage&tool=neo"`のようなパスでお絵かきアプリからのデータ受信できるようになりました。    
