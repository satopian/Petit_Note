...
- この問題はChickenPaintをフルスクリーンモードで起動している時のみ発生します。
- そのため、ChickenPaintのフルスクリーンモードでの起動オプションを外し、起動時にはミニスクリーンモードになるようにしました。 
これは、フルスクリーンモードで起動するとミニスクリーンに切り替える事ができなくなるからです。  
...
しかし、その中身は空白の記事が検索画面に表示されていました。   
中身が空白の記事の時は検索画面に表示されないようにしました。  
...
しかし、1ページ中の途中に閉じたスレッドが存在すると、その位置から下のスレッドのボタンが｢このスレッドを表示｣担ってしまうバグが見つかりました。
