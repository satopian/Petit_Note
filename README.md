...
ChickenPaintのショートカットキーを｢CTRL｣+｢+｣でキャンバスが拡大するように独自に修正しました。
本来のショートカットキーは、usキーボードで｢CTRL｣+｢=｣がキャンバスの拡大で、日本語キーボードでは、｢CTRL｣+｢;｣がキャンバスの拡大です。  
...
キャッシュは記事になんらかの変更があった時に破棄され、最初にページが表示された時に計算結果がキャッシュに保存されます。
